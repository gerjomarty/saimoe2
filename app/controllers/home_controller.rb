  ...
  def index
    @most_recent_view_models = Match.ordered_by_date
                                    .where(is_finished: true,
                                           date: Match.where(is_finished: true).maximum(:date)).collect do |m|
                                             MatchViewModel.new(m, cache: :home_most_recent, match_name: :long, info_position: :bottom, show_percentages: true)
                                           end
    ...
    @todays_match_view_models =
      if ((scope = Match.ordered_by_date.where(is_finished: false, date: Time.zone.now.to_date).scoped).any?) ||
        ...
        scope.collect {|m| MatchViewModel.new(m, cache: :home_todays_matches, match_name: :long, info_position: :bottom) }
      else
        nil
      end
